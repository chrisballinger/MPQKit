...
int Compress_copy(char * pbOutBuffer, int * pdwOutLength, char * pbInBuffer, int dwInLength, int * /* pCmpType */, int /* nCmpLevel */)
{
...
int Compress_bz2(char * pbOutBuffer, int * pdwOutLength, char * pbInBuffer, int dwInLength, int * /* pCmpType */, int nCmpLevel)
{
    ...
    *pdwOutLength = 0;
	...
	while(nResult != BZ_STREAM_END && s.avail_in > 0 && s.next_out > 0) {
		nResult = BZ2_bzCompress(&s, BZ_FINISH);
	}
        
	if(nResult == BZ_STREAM_END) {
		*pdwOutLength = s.total_out_lo32;
	}

	nResult = BZ2_bzCompressEnd(&s);
    return nResult;
}
...
static TCompressTable cmp_table[] =
{
	...
	{0x10, Compress_bz2}				// bz2 compression
};
...
static TDecompressTable dcmp_table[] = {
	{0x10, Decompress_bz2},				// Decompression with the bz2 library
