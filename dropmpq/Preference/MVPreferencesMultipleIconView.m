...
- (void) _drawIconAtIndex:(unsigned int) index drawRect:(NSRect) drawRect {
	...
	//NSPoint drawPoint;
	float nameHeight;
	NSRect buttonRect, destinationRect, nameRect;
	...
	attributesDictionary = [NSDictionary dictionaryWithObjectsAndKeys:[NSFont toolTipsFontOfSize:11.], NSFontAttributeName, paraStyle, 
        NSParagraphStyleAttributeName, nil];
	
	NSLayoutManager* lm = [[NSLayoutManager alloc] init];
    nameHeight = [lm defaultLineHeightForFont:[NSFont toolTipsFontOfSize:11.]];
	[lm release];
	
    /*drawPoint = NSMakePoint( rint( buttonSize.width + 2 * NSMinX( buttonRect ) - NSWidth( _bounds ) - 1 ), 
        rint( NSMaxY( buttonRect ) - titleBaseline - nameHeight ) );*/
    
    //[name drawAtPoint:drawPoint withAttributes:attributesDictionary];
    nameRect = NSMakeRect( NSMinX( buttonRect ), NSMaxY( buttonRect ) - titleBaseline - nameHeight, NSWidth( buttonRect ), 2*(nameHeight+3));
    
    [name drawInRect:nameRect withAttributes:attributesDictionary];
    [paraStyle release];
}
...
- (BOOL) _dragIconImage:(NSImage *) iconImage andName:(NSString *) name andIdentifier:(NSString *) identifier event:(NSEvent *) event {
	...
	[dragImage lockFocus]; {
		...
		[iconImage drawInRect:NSMakeRect( buttonSize.width / 2. - iconSize.width / 2., iconBaseline, iconSize.width, iconSize.height ) 
            fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:.5];

		[paraStyle setAlignment:NSCenterTextAlignment];
		NSLayoutManager* lm = [[NSLayoutManager alloc] init];
		attributesDictionary = [NSDictionary dictionaryWithObjectsAndKeys:[NSFont toolTipsFontOfSize:11.], NSFontAttributeName, 
            [[NSColor blackColor] colorWithAlphaComponent:.5], NSForegroundColorAttributeName, paraStyle, NSParagraphStyleAttributeName, nil];
        ...
        nameRect = NSMakeRect( 0, 0, buttonSize.width, 2*(nameHeight+3));
        [name drawInRect:nameRect withAttributes:attributesDictionary];
