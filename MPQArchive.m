...
- (BOOL)addArrayToFileList:(NSArray *)listfile error:(NSError **)error {
    ...
    while ((listfileEntry = [listfileEnumerator nextObject])) {
        ...
        free(filename_cstring);
        if (!result) ReturnValueWithError(NO, MPQErrorDomain, errOutOfMemory, nil, error)
    }
...
- (BOOL)addFileWithData:(NSData *)data filename:(NSString *)filename parameters:(NSDictionary *)parameters error:(NSError **)error {
    ...
    // Manage an autorelease pool to flush all the temp. objects generated by this method
    NSAutoreleasePool *p = [NSAutoreleasePool new];
...
WriteFailed:
    ...
    [p release];
    if (error && *error == nil) *error = [NSError errorWithDomain:MPQErrorDomain code:errUnknown userInfo:NULL];
    return NO;
}
